...
Useful features for recommending entities are tabulated in \tabref{tab:nb-analysis}.
\begin{table}[htbp]
  ...
  \end{tabular}}
  \caption{Highly weighted features for recommending \textsc{PER} entities according to relationship criterion.  For example, we found that if the graph claimed that someone was a resident of ``Russian'' or employed by ``Russian'', then their origin might likely be ``Russia''.}
  \end{adjustwidth}
