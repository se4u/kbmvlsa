...
def read_query_file(query_fn, name_to_idx_map):
    ...
    for (positive, negative) in zip(query_file_data[::2], query_file_data[1::2]):
        ...
        knownRed_idx = [name_to_idx_map[e]
                        for e in knownRed_name if e in name_to_idx_map]
        knownNotRed_idx = [name_to_idx_map[e]
                           for e in knownNotRed_name if e in name_to_idx_map]
...
def write_to_file(f, category, values, vertex_names):
    f.write(category)
    f.write(' ')
    f.write(' '.join('%s=%.3f' % (nm, v)
                     for (nm, v) in zip(vertex_names, values)))
    f.write('\n')
    f.flush()
...
def get_features_and_name_from_r_file(vertex_feature_fn, vertex_name_fn):
    ...
    vertex_features = rpy2_matrix_to_numpy_matrix(
        rpy2.robjects.r['vertex.features'])
...
def main():
    ...
    pickle.dump(training_acc, open(
        args.output_fn_prefix + 'training_acc.pickle', 'wb'), protocol=-1)
    return
...
if __name__ == '__main__':
    ...
    arg_parser.add_argument(
        '--vertex_name_fn', default="/export/projects/prastogi/kbvn/Kelvin.BinaryCoMentionGraph.vertexnames", type=str)
    arg_parser.add_argument(
        '--query_fn', default="/home/hltcoe/ngao/miniScale-2016/Kelvin/VNSeeds", type=str)
    arg_parser.add_argument(
        '--output_fn_prefix', default=_fn + '_logistic_', type=str)
    ...
    import rasengan
    args.output_fn_prefix = args.output_fn_prefix.replace(
        'logistic_probability', 'logistic_')
    print args
